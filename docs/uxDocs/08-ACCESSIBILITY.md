...
<div role="tablist" aria-label="Project views">
  <button role="tab" id="tab-board" aria-selected="true" aria-controls="panel-board" tabindex="0">
    Board
  </button>
  <button role="tab" id="tab-list" aria-selected="false" aria-controls="panel-list" tabindex="-1">
    List
  </button>
</div>

<div role="tabpanel" id="panel-board" aria-labelledby="tab-board" tabindex="0">
  <!-- Board content -->
</div>
```
...
<div role="group" aria-label="Task board: Sprint 14">
  <div role="region" aria-label="Backlog, 6 items">
    ...
    <div role="list" aria-label="Backlog tasks">
      <div
        role="listitem"
        aria-label="SIO-231, Migrate DB schema, Priority P0, Assigned to Marcus, Due July 10"
      >
        <!-- Card content -->
      </div>
    </div>
...
<div role="toolbar" aria-label="Formatting" aria-orientation="horizontal">
  ...
  <button aria-label="Insert link">🔗</button>
</div>
```
...
<div role="status" aria-live="polite">Task SIO-245 updated successfully.</div>

<!-- Error toast (interrupts user) -->
<div role="alert" aria-live="assertive">Failed to save changes. Please try again.</div>

<!-- Progress toast -->
<div role="status" aria-live="polite" aria-busy="true">Importing 150 tasks... (42% complete)</div>
```
...
<div class="form-field">
  ...
  <input id="task-title" type="text" aria-required="true" aria-describedby="task-title-hint" />
  <span id="task-title-hint" class="hint"> Required. Max 256 characters. </span>
</div>
```
