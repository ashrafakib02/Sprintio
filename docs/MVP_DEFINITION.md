...
| Feature Requests (organic) | Evidence users want _more_, not _different_  |
...
_This document is the single source of truth for what ships in Sprintio v1.0. Any feature not listed as "Ships" in Section 2 is explicitly excluded from MVP scope. Scope additions require an equal-scope deferral and approval from the Approvers listed above._
